...
function CommandsCharts({ dataCommands }) {
  ...
  let emptyMessage = "Préparation des données en cours...";

  if (!dataCommands || !Array.isArray(dataCommands)) {
    emptyMessage = "Erreur de format des données";
  } else if (dataCommands.length === 0) {
    ...
    <div className="chart-surface p-6">
      ...
      <div className="mt-6">
        {chartData && chartData.length > 0 ? (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {chartData.map((item) => (
                <div
                  key={item.name}
                  className="bg-slate-100/5 rounded-lg p-4 border border-slate-100/10"
                >
                  ...
                  <div className="text-slate-400 text-xs">
                    {totalCommands > 0
                      ? Math.round((item.value / totalCommands) * 100)
                      : 0}
                    % du total
                  </div>
                </div>
          ...
          <EmptyState title="Aucune donnée à afficher" message={emptyMessage} />
        )}
      </div>
    </div>
  );
...
CommandsCharts.propTypes = {
  dataCommands: PropTypes.arrayOf(PropTypes.object),
};
