...
function mapCarToForm(car) {
  return {
    ...
    imageFile: null,
    disponible: car?.disponible === undefined ? true : car.disponible,
  };
}

function EditCarsForm({ car, onSave, onCancel }) {
  const [formData, setFormData] = useState(() => mapCarToForm(car));

  useEffect(() => {
    if (car) {
      setFormData(mapCarToForm(car));
    }
  }, [car]);
