...
function getPaymentMethodClass(method) {
  return paymentMethodClasses[method] || paymentMethodClasses.default;
}
...
function CommandInfoCard(props) {
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`px-3 py-1 text-xs rounded-full font-medium ${getStatusBadgeClass(command.statut)}`}
              >
                {command.statut}
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between border-t border-slate-700 pt-2">
                ...
                <span className="text-emerald-400 font-bold text-lg">
                  {addThousandSeparator(
                    command.montantTotal || command.montant || 0,
                  )}{" "}
                  FCFA
                </span>
              </div>
              ...
              <div className="mt-2">
                <span
                  className={`px-2 py-1 text-xs rounded-full ${getPaymentMethodClass(command.modePaiement)}`}
                >
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                <LuMapPin className="text-slate-400 text-sm mt-0.5 shrink-0" />
                <div>
                  <p className="text-slate-400 text-xs font-medium">
                    Adresse de livraison:
                  </p>
                  <p className="text-slate-200 text-sm">
                    {formatAddress(command.adresseLivraison)}
                  </p>
                </div>
              </div>

              <div className="flex items-start gap-2">
                <LuCalendar className="text-slate-400 text-sm mt-0.5 shrink-0" />
                <div>
                  <p className="text-slate-400 text-xs font-medium">
                    Date commande:
                  </p>
                  <p className="text-slate-200 text-sm">
                    {formatDate(command.dateCommande)}
                  </p>
                </div>
              </div>
              ...
              {command.dateLivraisonPrevue && (
                <div className="flex items-start gap-2">
                  <LuTruck className="text-slate-400 text-sm mt-0.5 shrink-0" />
                  <div>
                    <p className="text-slate-400 text-xs font-medium">
                      Livraison prévue:
                    </p>
                    <p className="text-slate-200 text-sm">
                      {formatDate(command.dateLivraisonPrevue)}
                    </p>
                  </div>
                </div>
              )}
              ...
              {command.dateLivraisonReelle && (
                <div className="flex items-start gap-2">
                  <LuTruck className="text-green-400 text-sm mt-0.5 shrink-0" />
                  <div>
                    <p className="text-slate-400 text-xs font-medium">
                      Livré le:
                    </p>
                    <p className="text-green-300 text-sm font-medium">
                      {formatDate(command.dateLivraisonReelle)}
                    </p>
                  </div>
                </div>
              )}
...
CommandInfoCard.propTypes = {
  command: PropTypes.shape({
    ...
    montantTotal: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    fraisLivraison: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    modePaiement: PropTypes.string,
    adresseLivraison: PropTypes.shape({
      rue: PropTypes.string,
      ville: PropTypes.string,
      codePostal: PropTypes.string,
    }),
    dateCommande: PropTypes.string,
    dateLivraisonPrevue: PropTypes.string,
    dateLivraisonReelle: PropTypes.string,
