              ...
              {paginatedFavorites.map((item) => (
                ...
                >
                  <div className="rounded-lg overflow-hidden h-36 mb-3 client-theme-card">
                    {item.voiture?.images?.[0] ? (
                      <img
                        src={uploadService.resolveImageUrl(item.voiture.images[0])}
                        alt={`${item.voiture?.marque ?? "Voiture"} ${item.voiture?.modele ?? ""}`.trim()}
                  ...
                  <p className="client-theme-value font-medium mt-2">
                    {Number(item.voiture?.prix ?? 0).toLocaleString("fr-FR")} FCFA
                  </p>
