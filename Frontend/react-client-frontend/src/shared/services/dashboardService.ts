...
class DashboardService {
    ...
    async getUserEnhancedData(token: string): Promise<EnhancedUser> {
        const data = await this.makeRequest<UserDataResponse>(
            API_PATHS.USER.GET_USER_ENHANCED,
            token
        );
        return data.user;
    }
    ...
    async getUserAchats(token: string, page: number = 1, limit: number = 10): Promise<AchatsResponse> {
        try {
    ...
    async getUserFavorites(token: string, page: number = 1, limit: number = 10): Promise<FavorisResponse> {
        try {
    ...
    async addEvaluationToAchat(token: string, achatId: string, evaluation: EvaluationData): Promise<AchatsResponse> {
        ...
        const data = await response.json();
        if (!data.success) {
            throw new Error(data.message || "Erreur lors de l'ajout de l'évaluation");
        }
        return data;
    }
    ...
    handleError(error: unknown): string {
        if (typeof error === 'object' && error !== null && 'message' in error) {
            // @ts-expect-error: TS can't know error has message
