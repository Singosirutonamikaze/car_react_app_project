...
const getClients = async (_req, res) => {
    try {
        const clients = await Client_1.default.find().select('-password');
        res.json(clients);
    }
    catch (error) {
...
const updateClient = async (req, res) => {
    try {
        ...
        const client = await Client_1.default.findByIdAndUpdate(_id, updates, { new: true, runValidators: true }).select('-password');
        if (!client) {
            res.status(404).json({
...
const downloadClientsPDF = async (_req, res) => {
    try {
        const clients = await Client_1.default.find().select('-password');
